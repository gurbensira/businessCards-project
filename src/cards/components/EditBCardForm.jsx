...
import createInitialFormFromCard from "../helpers/mappers/cardFormMapper";
...
function EditBCardForm({ onClick, onCardUpdated, card }) {
    ...
    const handleUpdateCard = async (cardDetails) => {
        try {
            ...
            setSnack("success", "Business card updated successfully!");
            ...
            const errorMessage = error.response?.data || "Failed to update card. Please try again.";
            setSnack("error", errorMessage);
        }
    };

    const { formDetails, errors, handleChange, handleSubmit, setFormDetails } = useForm(
        createInitialFormFromCard(card),
        bcardSchema,
        handleUpdateCard,
        setSnack
    );
    ...
    useEffect(() => {
        if (card) {
            setFormDetails(createInitialFormFromCard(card));
        }
    }, [card, setFormDetails]);
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Form
                    ...
                    onReset={() => setFormDetails(createInitialFormFromCard(card))}
